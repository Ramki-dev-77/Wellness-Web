...
public class QrService {
    ...
    public Map<String, Object> generateHealthRecordsQR(Map<String, Object> workerData, Map<String, Object> healthRecords) {
        try {
            ...
            String patientId = (String) workerData.get("fhirPatientId");
            String recordsUrl = String.format("http://localhost:8081/doctor/records/patient/%s", patientId);
    ...
    public Map<String, Object> generateQRForWorker(User worker) {
        try {
            ...
            Map<String, Object> workerData = Map.of(
                    "abha", Optional.ofNullable(worker.getAbhaNumber()).orElse("N/A"),
                    "name", Optional.ofNullable(worker.getName()).orElse("Unknown Worker"),
                    "mobile", Optional.ofNullable(worker.getMobile()).orElse("N/A"),
                    "region", Optional.ofNullable(worker.getRegion()).orElse("Unknown Region"),
                    "fhirPatientId", Optional.ofNullable(worker.getFhirPatientId()).orElse("N/A")
            );
    ...
    public Map<String, Object> decryptQRData(String encryptedData) {
        try {
            if (encryptedData == null || encryptedData.trim().isEmpty()) {
                return Map.of("success", false, "error", "Encrypted data is required");
            }
    ...
    // Additional method for generating QR with custom data
    public Map<String, Object> generateQRWithCustomData(Map<String, Object> customWorkerData,
                                                        Map<String, Object> customHealthRecords) {
        try {
